...
import gov.faa.ang.swac.common.random.util.*;
...
public void setMax(double max) {
   ...
   protected double min, max;          // Interval x in [a, b]
   protected double bminusa;
   protected double logFactor;
   //protected double tempBeta;          // Function Beta(alpha, beta)
   protected double logBeta;       // Ln(Beta(alpha, beta))

   /**
    * Constructs a <TT>BetaDistribution</TT> object with parameters <SPAN CLASS="MATH"><I>&#945;</I> =</SPAN>
    ...
    * 
    */
   public BetaDistribution (double alpha, double beta, double a, double b) {
   ...
   /**
    * Constructs a <TT>BetaDistribution</TT> object with parameters 
    ...
    * 
    */
   public BetaDistribution (double alpha, double beta, int d) {
    ...
    * 
    */
   public BetaDistribution (double alpha, double beta, double a, double b, int d) {
   ...
   public double density (double x) {
      if (x <= min || x >= max)
         return 0;
      double temp = (alpha - 1) * Math.log(x - min) + (beta - 1) * Math.log(max - x);
//      return factor*Math.pow (x - a, alpha - 1)*Math.pow (b - x, beta - 1);
      return Math.exp(logFactor + temp);
   }

   public double cdf (double x) {
      return cdf (alpha, beta, decPrec, (x - min)/bminusa);
   }

   public double nextDouble (Random rng) {
      return min + (max - min)*inverseF (alpha, beta,  decPrec, rng.nextDouble());
   }

   public double getMean() {
      return BetaDistribution.getMean (alpha, beta, min, max);
   }

   public double getVariance() {
      return BetaDistribution.getVariance (alpha, beta, min, max);
   }

   public double getStandardDeviation() {
      return BetaDistribution.getStandardDeviation (alpha, beta, min, max);
   }
 ...
 */
    ...
    */
      ...
      if ((alpha > ALPHABETAMAX && beta < ALPHABETALIM) ||
         ...
         if (flag) {
            yd = GammaDistribution.barF (beta, d, yd);
            return yd - gam;
         } else {
            yd = GammaDistribution.cdf (beta, d, yd);
            return yd + gam;
         }
      }
      ...
      return NormalDistribution.cdf01 (h3);
   }
            ...
            for (i = 0; i<100; i++) {
               if (i != 0) {
                  ...
                  if (x == 0.0) {
                     ...
                     x = x0  +  di*(x1 - x0);
                     if (x == 0.0) {
                        // System.err.println ("BetaDistribution.inverseF: underflow");
                        return 0;
                     }
                  }
                  y = cdf (p, q, d, x);
...
//               y = EPS;
         ...
         yp = -NormalDistribution.inverseF01 (u);
...
*/
    ...
    * 
    */
/*   public static BetaDistribution getInstanceFromMLE (double[] x, int n) {
      double parameters[] = getMLE (x, n);
      return new BetaDistribution (parameters[0], parameters[1]);
   }
*/
    ...
    *  ALIGN="MIDDLE" BORDER="0" SRC="BetaDistributionimg1.png"
    ...
    *    
    * <SPAN CLASS="MATH">Var[<I>X</I>] = <IMG
    *  ALIGN="MIDDLE" BORDER="0" SRC="BetaDistributionimg2.png"
   ...
   public static double getStandardDeviation (double alpha, double beta)  {
      return Math.sqrt (BetaDistribution.getVariance (alpha, beta));
   }
   ...
   public static double getStandardDeviation (double alpha, double beta,
                                              double a, double b)  {
      return Math.sqrt (BetaDistribution.getVariance (alpha, beta, a, b));
   }
...
public void setBminusa(double bminusa) {
...
//      tempBeta = Math.exp(logBeta);
//      this.factor = 1.0 / (tempBeta * Math.pow (bminusa, alpha + beta - 1));
