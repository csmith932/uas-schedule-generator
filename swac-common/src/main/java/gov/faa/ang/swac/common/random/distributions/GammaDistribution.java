...
import gov.faa.ang.swac.common.random.functions.MathFunction;
import gov.faa.ang.swac.common.random.util.RootFinder;
...
public class GammaDistribution extends ContinuousDistribution {
   ...
   private static class myFunc implements MathFunction {
      ...
      public double evaluate (double x) {
         return u - GammaDistribution.cdf(alp, d, x);
      }
   }
   ...
   /**
    * Constructs a <TT>GammaDistribution</TT> object with parameters
    ...
    * 
    */
   public GammaDistribution (double alpha, double lambda) {
      setParams (alpha, lambda, decPrec);
   }


   /**
    * Constructs a <TT>GammaDistribution</TT> object with parameters <SPAN CLASS="MATH"><I>&#945;</I></SPAN> =
    ...
    * 
    */
   public GammaDistribution (double alpha, double lambda, int d) {
      setParams (alpha, lambda, d);
   }

   public void setShape(String alpha){
	   this.shape = Double.parseDouble(alpha);
   }
...
private static double mybelog (double x)
   ...
   public double density (double x) {
      if (x <= 0)
         return 0.0;
      double z = logFactor + (shape - 1.0) * Math.log(x) - rate * x;
      if (z > -XBIGM)
         return Math.exp (z);
      else
         return 0.0;
   ...
   public double cdf (double x) {
      return cdf (shape, rate, decPrec, x);
   }

   public double barF (double x) {
      return barF (shape, rate, decPrec, x);
   }

   public double nextDouble (Random rng) {
      return inverseF (shape, decPrec, rng.nextDouble())/rate;
   }

   public double getMean() {
      return GammaDistribution.getMean (shape, rate);
   }

   public double getVariance() {
      return GammaDistribution.getVariance (shape, rate);
   }

   public double getStandardDeviation() {
      return GammaDistribution.getStandardDeviation (shape, rate);
   }
   ...
   public static double cdf (double alpha, int d, double x) {
      ...
      if (x <= 0.0)
         return 0.0;
      if (1.0 == alpha)
         return ExponentialDistribution.cdf (1.0, x);
      ...
      if (alpha >= ALIM) {
         ...
         return NormalDistribution.cdf01 (y);
      }
   ...
   public static double barF (double alpha, int d, double x) {
      ...
      if (x <= 0.0)
         return 1.0;
      if (1.0 == alpha)
         return ExponentialDistribution.barF (1.0, x);
      ...
      if (alpha >= ALIM) {
         ...
         double w = mybelog(S/x);
         double y = d2 * Math.sqrt(w/x);
         return NormalDistribution.barF01 (y);
      }
   ...
   public static double inverseF (double alpha, int d, double u) {
      ...
      double sigma = GammaDistribution.getStandardDeviation (alpha, 1.0);
      double x = NormalDistribution.inverseF (alpha, sigma, u);
      double v = GammaDistribution.cdf (alpha, d, x);
      double xmax;
      if (alpha < 1.0)
         xmax = 100.0;
      else
    ...
    * 
    */
   public static GammaDistribution getInstanceFromMLE (double[] x, int n) {
   ...
   public double[] getParams () {
      double[] retour = {shape, rate};
      return retour;
   }


   public String toString () {
      return getClass().getSimpleName() + " : alpha = " + shape + ", lambda = " + rate;
   }
