 ...
 * 
 */
public class PoissonDistribution extends DiscreteDistributionInt {
   ...
   public double cdf (int x) {
      ...
      if (cdf == null)
         return GammaDistribution.barF (x + 1.0, 15, lambda);
      ...
      if (x < xmin) {
         ...
         // could also call GammaDistribution.barF instead.
         final int RMAX = 20;
         int i;
   ...
   public double barF (int x) {
      ...
      /* For large lambda,  we use the Chi2 distribution according to the exact
         ...
         cdf (lambda, x) = 1 - GammaDistribution.cdf (x + 1, lambda)
       */

      if (cdf == null)
         return GammaDistribution.cdf ((double)x, 15, lambda);

      if (x > xmax)
//         return GammaDistribution.cdf ((double)x, 15, lambda);
         return PoissonDistribution.barF(lambda, x);
      if (x <= xmin)
         return 1.0;
      if (x > xmed)
         // We keep the complementary distribution in the upper part of cdf
   ...
   public double nextDouble(Random rng){
	  return inverseFInt(rng.nextDouble()); 
   }
   
   public double getMean() {
      return PoissonDistribution.getMean (lambda);
   }

   public double getVariance() {
      return PoissonDistribution.getVariance (lambda);
   }

   public double getStandardDeviation() {
      return PoissonDistribution.getStandardDeviation (lambda);
   }
    ...
    */
      ...
      if (x >= lambda)
         return 1 - PoissonDistribution.barF(lambda, x+1);
   ...
   public static double barF (double lambda, int x) {
      ...
      /* If lambda > LAMBDALIM, we use the Chi2 distribution according to the
         ...
         which also equals   1 - GammaDistribution.cdf (x + 1, lambda) */
      ...
      if (x <= lambda)
         return 1.0 - PoissonDistribution.cdf(lambda, x - 1);
      ...
      sum = term = PoissonDistribution.prob(lambda, x);
      int i = x + 1;
   ...
   public static int inverseF (double lambda, double u) {
      ...
      if (lambda < LAMBDALIM) {
         ...
         i = (int)lambda;
         double term = PoissonDistribution.prob(lambda, i);
         while ((term >= u) && (term > Double.MIN_NORMAL)) {
            i /= 2;
            term = PoissonDistribution.prob (lambda, i);
         }
    ...
    * 
    */
   public static PoissonDistribution getInstanceFromMLE (int[] x, int n) {
      double parameters[] = getMLE (x, n);
      return new PoissonDistribution (parameters[0]);
   }
